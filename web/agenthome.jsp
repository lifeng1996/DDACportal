...
function searchfromto() {
  ...
  var xhttp;    
  if (str == "") {
    alert('Please Enter Destination before search');
    return;
  }else if(str1==""){
      alert('Please Enter Source before search');
    return;
  }
  ...
  xhttp.open("GET", "agentsearchschedule.jsp?source="+str1+"&destination="+str, true);
  xhttp.send();
}
</script>
                            </div>
      ...
      if (td.innerHTML.toUpperCase().indexOf(filter) > -1) {
        ...
        tr[i].style.display = "none";
               ...
               <%
                   ...
                   while(rs.next()){
                       if(rs.getString("STATUS").equalsIgnoreCase("Approve")){
                           x++;
                           out.println(x+". Booking_ID: "+rs.getString("BID")+" has been approved! <br>");
                       }else if(rs.getString("STATUS").equalsIgnoreCase("Decline")){
                           x++;
                           out.println(x+". Booking_ID: "+rs.getString("BID")+" has been declined!<br>");
                       }
                   }
                rs.close();
                stmt.close();
